...
export const formReducer = createReducer(
  ...
  on(inputUpdate, (state, inputUpdate) => ({
    ...state,
    inputs: [
  ...
  on(textAreaUpdate, (state, textAreaUpdate) => ({
    ...state,
    textAreas: [
      ...state.textAreas.map(textArea => {
        return textArea.id === textAreaUpdate.id ? textAreaUpdate : textArea;
      }),
    ],
  })),
  on(buttonUpdate, (state, buttonUpdate) => ({
    ...state,
    buttons: [
  ...
  on(checkBoxUpdate, (state, checkBoxUpdate) => ({
    ...state,
    checkBoxes: [
  ...
  on(selectUpdate, (state, selectUpdate) => ({
    ...state,
    selects: [
    ...
    selects: [
      ...state.selects.map(select => {
        return select.id === option.id
          ? {
              ...select,
              selectAddOption: [...select.selectAddOption, option.option],
            }
          : select;
      }),
    ],
